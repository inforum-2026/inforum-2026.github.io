---
title: "Programa"
---

## Programa Provisório

**Artigos:** 20 minutos (15 min apresentação + 5 min discussão)
**Comunicações:** 12 minutos (10 min apresentação + 2 min discussão)

<div class="schedule-day">

### 3 de Setembro (Quinta-feira)

<table class="schedule-table">
  <thead>
    <tr>
      <th class="col-time">Hora</th>
      <th class="col-room">Sala 1</th>
      <th class="col-room">Sala 2</th>
      <th class="col-room">Sala 3</th>
    </tr>
  </thead>
  <tbody>
    <tr class="schedule-plenary">
      <td>11:00</td>
      <td colspan="3">Registo</td>
    </tr>
    <tr class="schedule-social">
      <td>12:00</td>
      <td colspan="3">Almoço</td>
    </tr>
    <tr class="schedule-plenary">
      <td>13:20</td>
      <td colspan="3">Abertura</td>
    </tr>
    <tr class="schedule-session">
      <td>13:30</td>
      <td><a href="#cpdla1">CPDLA (1)</a></td>
      <td><a href="#iacd1">IACD (1)</a></td>
      <td><a href="#softpt1">SOFT-PT (1)</a></td>
    </tr>
    <tr class="schedule-break">
      <td>15:15</td>
      <td colspan="3">Coffee Break</td>
    </tr>
    <tr class="schedule-plenary">
      <td>15:30</td>
      <td colspan="3">Keynote - A Whirlwind Tour of Open Problems in AI
  Security, Fred Oliveira</td>
    </tr>
    <tr class="schedule-session">
      <td>16:45</td>
      <td><a href="#sscc1">SSCC (1)</a></td>
      <td><a href="#iacd2">IACD (2)</a></td>
      <td>—</td>
    </tr>
    <tr class="schedule-social">
      <td>20:00</td>
      <td colspan="3">Jantar</td>
    </tr>
  </tbody>
</table>

</div>

<div class="schedule-day">

### 4 de Setembro (Sexta-feira)

<table class="schedule-table">
  <thead>
    <tr>
      <th class="col-time">Hora</th>
      <th class="col-room">Sala 1</th>
      <th class="col-room">Sala 2</th>
      <th class="col-room">Sala 3</th>
    </tr>
  </thead>
  <tbody>
    <tr class="schedule-plenary">
      <td>09:00</td>
      <td colspan="3">Keynote - From Conflict Resolution To Application
  Correctness in Distributed Systems, Nuno Preguiça</td>
    </tr>
    <tr class="schedule-break">
      <td>10:15</td>
      <td colspan="3">Coffee Break</td>
    </tr>
    <tr class="schedule-session">
      <td>10:30</td>
      <td><a href="#cpdla2">CPDLA (2)</a></td>
      <td><a href="#iacd3">IACD (3)</a></td>
      <td><a href="#softpt2">SOFT-PT (2)</a></td>
    </tr>
    <tr class="schedule-social">
      <td>12:15</td>
      <td colspan="3">Almoço</td>
    </tr>
    <tr class="schedule-plenary">
      <td>13:30</td>
      <td colspan="3">Industry Session</td>
    </tr>
    <tr class="schedule-session">
      <td>14:00</td>
      <td><a href="#sscc2">SSCC (2)</a></td>
      <td><a href="#iacd4">IACD (4)</a></td>
      <td><a href="#sci">SCI</a></td>
    </tr>
    <tr class="schedule-break">
      <td>15:30</td>
      <td colspan="3">Coffee Break + Pósters + Demos</td>
    </tr>
    <tr class="schedule-plenary">
      <td>16:30</td>
      <td colspan="3">Business Meeting</td>
    </tr>
    <tr class="schedule-plenary">
      <td>17:30</td>
      <td colspan="3">Encerramento</td>
    </tr>
  </tbody>
</table>

</div>

---

## Sessões

### <span id="cpdla1">CPDLA (1) — Quinta-feira, 13:30</span>

<table class="session-table">
  <tbody>
    <tr><td class="session-time">13:30</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Evaluating the Practicality of Learned Cardinality Estimators in Query Optimizers</span><br><span class="paper-authors">Luís França e José Pereira</span></td></tr>
    <tr><td class="session-time">13:50</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">FLeeC+: A Fully Lock-Free High-Performance Caching System</span><br><span class="paper-authors">Samuel C. Fernão Pires, André Costa e João M. Lourenço</span></td></tr>
    <tr><td class="session-time">14:10</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Recuperação de Faltas em Sistemas de Disseminação Bizantinos Suportados por Árvores</span><br><span class="paper-authors">Alexandre Ferreira, Luís Rodrigues e Miguel Matos</span></td></tr>
    <tr><td class="session-time">14:30</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Utilização de Históricos Partilhados para Suporte a Sistemas de Actores</span><br><span class="paper-authors">Rafael Sargento, Rafael Soares e Luís Rodrigues</span></td></tr>
    <tr><td class="session-time">14:50</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Sharing Intel Data Streaming Accelerator (DSA) between virtual machines</span><br><span class="paper-authors">Nuno Alves e João Barreto</span></td></tr>
    <tr><td class="session-time">15:02</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Decoupling BFT Consensus via Monotonic Merges and Deterministic Projection</span><br><span class="paper-authors">Francisco Silveira e António Ravara</span></td></tr>
  </tbody>
</table>

### <span id="iacd1">IACD (1) — Quinta-feira, 13:30</span>

<table class="session-table">
  <tbody>
    <tr><td class="session-time">13:30</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Hybrid Gaussian-TCN time series forecasting model applied for insulator fault prediction</span><br><span class="paper-authors">António Pires, Stefano Stefenon e Rui Jesus</span></td></tr>
    <tr><td class="session-time">13:50</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Contrastive Clustering for Multimodal Longitudinal Subtyping of Parkinson's Disease Progression</span><br><span class="paper-authors">Diogo Barreta e Diogo Soares</span></td></tr>
    <tr><td class="session-time">14:10</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Descriptive and Predictive Modeling of Blood Activity in Portugal. What has changed since COVID-19?</span><br><span class="paper-authors">Diogo Pires, Pedro Monteiro e Rui Henriques</span></td></tr>
    <tr><td class="session-time">14:30</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Detecting AI-Generated Speech: A Comparative Study of Neural Network Approaches</span><br><span class="paper-authors">Sebastião Teixeira, Rui Machado, Tiago Almeida e Sérgio Matos</span></td></tr>
    <tr><td class="session-time">14:50</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Benchmarking CNN-Sequential Architectures for Raw Nanopore Signal AMR Classification: GAP vs GRU</span><br><span class="paper-authors">Leonardo Fonseca, Juan Aldana e Melanie Cujilema</span></td></tr>
    <tr><td class="session-time">15:02</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Data Analytics for Pediatric Palliative Care in Portugal</span><br><span class="paper-authors">Tiago Nóbrega, Maria João Palaré, Alexandre Corte, Daniel Gonçalves, Pedro T. Monteiro e Helena Galhardas</span></td></tr>
  </tbody>
</table>

### <span id="softpt1">SOFT-PT (1) — Quinta-feira, 13:30</span>

<table class="session-table">
  <tbody>
    <tr><td class="session-time">13:30</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Typestated Monitors for Distributed Java Applications</span><br><span class="paper-authors">Francisco Parrinha, João Mota e António Ravara</span></td></tr>
    <tr><td class="session-time">13:50</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Minimization 2: Brzozowski Strikes Back</span><br><span class="paper-authors">Nelma Moreira, Rogério Reis e Gonçalo Teixeira</span></td></tr>
    <tr><td class="session-time">14:10</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Formal foundations for the Pocket+ compression algorithm and beyond</span><br><span class="paper-authors">Cláudia Faria, Patrícia Bastos e José Oliveira</span></td></tr>
    <tr><td class="session-time">14:30</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Even Camels Fall into Coma: Flexible Barriers for Deductive Verification of OCaml Programs</span><br><span class="paper-authors">Beatriz Rosas e Paul Patault</span></td></tr>
    <tr><td class="session-time">14:50</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Uma framework integrada para o desenvolvimento de CRDTs</span><br><span class="paper-authors">João Garção</span></td></tr>
  </tbody>
</table>

### <span id="iacd2">IACD (2) — Quinta-feira, 16:45</span>

<table class="session-table">
  <tbody>
    <tr><td class="session-time">16:45</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Efficient Online Clustering of Faces by Identity via Approximate Nearest-Neighbours</span><br><span class="paper-authors">Daniel Neves, Hervé Paulino e Nuno Marques</span></td></tr>
    <tr><td class="session-time">17:05</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Exploring Data Augmentation Strategies for Named Entity Recognition in Biomedical Literature</span><br><span class="paper-authors">Miguel Lourenço e Andre Lamurias</span></td></tr>
    <tr><td class="session-time">17:25</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Análise, Explicação e Recomendações de Incidentes de Cibersegurança: Uma Abordagem com dois Agentes</span><br><span class="paper-authors">Nuno Gomes, José Alegria e Artur Ferreira</span></td></tr>
    <tr><td class="session-time">17:45</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">A Friendly Personal Digital Assistant Supported by Large Language Models</span><br><span class="paper-authors">Dinis Dias, Nuno Leite e Artur Ferreira</span></td></tr>
  </tbody>
</table>

### <span id="sscc1">SSCC (1) — Quinta-feira, 16:45</span>

<table class="session-table">
  <tbody>
    <tr><td class="session-time">16:45</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">User Behavior Profiling via Kernel-Level Telemetry and Lightweight Edge-Inference</span><br><span class="paper-authors">João Realinho, Rui Claro, João Trindade e João M. Lourenço</span></td></tr>
    <tr><td class="session-time">17:05</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">An Initial Assessment of Chatbot Reliability in the Generation of Cybersecurity-Related Commands, Instructions, and Code</span><br><span class="paper-authors">Maria Inês Gomes, Pedro Inácio, Bernardo Sequeiros e Tiago Roxo</span></td></tr>
    <tr><td class="session-time">17:25</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Secure Distributed Aggregation Middleware for Challenging Environments</span><br><span class="paper-authors">João Barros, Alex Davidson e Vinicius Cogo</span></td></tr>
    <tr><td class="session-time">17:45</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Configuration Vulnerability Meter</span><br><span class="paper-authors">Alberto Filipe, Tadeu Freitas e João Soares</span></td></tr>
  </tbody>
</table>

### <span id="cpdla2">CPDLA (2) — Sexta-feira, 10:30</span>

<table class="session-table">
  <tbody>
    <tr><td class="session-time">10:30</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Reputação Tolerante a Falhas Bizantinas para Sistemas de Disseminação em Árvore</span><br><span class="paper-authors">Fábio Gomes, Luís Rodrigues e Miguel Matos</span></td></tr>
    <tr><td class="session-time">10:50</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Trade-offs in page promotion for multi-tiered memory systems</span><br><span class="paper-authors">Miguel Carvalho, Rodrigo Santos, Miguel Pereira, Ganliang Zhu, Arun Kp e João Barreto</span></td></tr>
    <tr><td class="session-time">11:10</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">GNN-Guided Graph Partitioning for Spatial CGRA Mapping</span><br><span class="paper-authors">Deniz Lopes Günes, Henrique dos Santos Teixeira, Guilherme Vareiro de Oliveira e Nuno Paulino</span></td></tr>
    <tr><td class="session-time">11:30</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Towards Distributed-Parallel Query Optimizers</span><br><span class="paper-authors">Luís Vítor Lima Barros, José Orlando Pereira e Nuno Filipe Pinto Faria</span></td></tr>
    <tr><td class="session-time">11:42</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Fast&amp;Securious: Permissioned Blockchain Application Performance Testing and Improvement</span><br><span class="paper-authors">João Durão, Miguel Pardal e Samih Eisa</span></td></tr>
    <tr><td class="session-time">11:54</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Cost-based Optimization of Queries in Distributed Data Processing</span><br><span class="paper-authors">Hélder Gomes e José Pereira</span></td></tr>
  </tbody>
</table>

### <span id="iacd3">IACD (3) — Sexta-feira, 10:30</span>

<table class="session-table">
  <tbody>
    <tr><td class="session-time">10:30</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Electricity Price Forecasting for Business Energy Contracting in Portugal: A Machine Learning Framework Integrated with Cost Simulation</span><br><span class="paper-authors">Ana Beatriz Pereira, João Pedro Pereira, João Silva, Sara Silva e Ana Maria de Almeida</span></td></tr>
    <tr><td class="session-time">10:50</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Design and Evaluation of a RAG-Based Chatbot for Project-Based Learning Support</span><br><span class="paper-authors">Maria Joana Pires e Mário Véstias</span></td></tr>
    <tr><td class="session-time">11:10</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Synthetic data for machine learning: a study on quality and evaluation</span><br><span class="paper-authors">Rebecca Cunha, Luís Nunes e Ana Maria de Almeida</span></td></tr>
    <tr><td class="session-time">11:30</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Safety Impact of SFT and DPO on Pre-Aligned Models</span><br><span class="paper-authors">Bernardo Leite, Celestino Goncalves e Clara Silveira</span></td></tr>
    <tr><td class="session-time">11:50</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">AI4MDR: Agentic AI for Medical Device Regulation Certification</span><br><span class="paper-authors">Júlia Bastos e Mário Zenha-Rela</span></td></tr>
  </tbody>
</table>

### <span id="softpt2">SOFT-PT (2) — Sexta-feira, 10:30</span>

<table class="session-table">
  <tbody>
    <tr><td class="session-time">10:30</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Type Inference Algorithms: a rewriting logic approach</span><br><span class="paper-authors">André Adrêgo, João Barbosa e Mário Florido</span></td></tr>
    <tr><td class="session-time">10:50</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Adversarially Verified Ephemeral Trust for Secure CI/CD Runners</span><br><span class="paper-authors">Paulo Duarte, Simão Melo de Sousa e Joel Guerreiro</span></td></tr>
    <tr><td class="session-time">11:10</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">How Developers Feel About AI Code Assistants: A Multi-Platform Study</span><br><span class="paper-authors">Miguel Cut, Diana Costa e Wellington Oliveira</span></td></tr>
    <tr><td class="session-time">11:30</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Pre-SMT Simplifications in Gillian: Usage, Cost, and Correctness</span><br><span class="paper-authors">Daniel Cruz</span></td></tr>
    <tr><td class="session-time">11:42</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Elevating Requirements Quality with LLMs: An Industrial Case Study</span><br><span class="paper-authors">Pedro Filipe, António Campos, J. Eduardo Ferreira Ribeiro e Mário Zenha-Rela</span></td></tr>
    <tr><td class="session-time">11:54</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Regular Expressions with Counting and Partial Derivatives</span><br><span class="paper-authors">Nelma Moreira, Rogério Reis e Susana Ribeiro</span></td></tr>
  </tbody>
</table>

### <span id="sscc2">SSCC (2) — Sexta-feira, 14:00</span>

<table class="session-table">
  <tbody>
    <tr><td class="session-time">14:00</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Salvaguarda e Re-execução para Detecção post-mortem de Ataques em Sistemas de Aprendizagem Automática Federada</span><br><span class="paper-authors">Mafalda Fernandes, Daniel Castro, Luís Rodrigues e Nuno Santos</span></td></tr>
    <tr><td class="session-time">14:20</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">LegionITS: A Federated Intrusion-Tolerant Architecture with Privacy-Preserving Threat Intelligence Sharing</span><br><span class="paper-authors">Tadeu Freitas, Carlos Novo, Manuel Correia e Rolando Martins</span></td></tr>
    <tr><td class="session-time">14:40</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Gestão Escalável de Proveniência em Ecossistemas de Aprendizagem Automática</span><br><span class="paper-authors">Daniel Ferreira, Daniel Castro, Luís Rodrigues e Nuno Santos</span></td></tr>
  </tbody>
</table>

### <span id="iacd4">IACD (4) — Sexta-feira, 14:00</span>

<table class="session-table">
  <tbody>
    <tr><td class="session-time">14:00</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Improving Deep Learning Models with Evolutionary Algorithms</span><br><span class="paper-authors">Ruben Café, Nuno Leite e Artur Ferreira</span></td></tr>
    <tr><td class="session-time">14:20</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Reproducible Machine Learning Baselines for Pediatric Obstructive Sleep Apnea Risk Stratification: A Leakage-Safe Pipeline with Patient-Grouped Evaluation</span><br><span class="paper-authors">Nuno Rodrigues e Isabel Machado Alexandre</span></td></tr>
    <tr><td class="session-time">14:40</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Approximating SPR Distance Between Phylogenetic Trees with Graph Neural Networks</span><br><span class="paper-authors">Renata Martins Castanheira, Miguel Bugalho e Cátia Vaz</span></td></tr>
    <tr><td class="session-time">15:00</td><td class="session-type session-comm">Comunicação</td><td><span class="paper-title">Controlled Graph-Based Ablations for Protein–Ligand Affinity Prediction: From Interaction Graphs to Coarse-to-Local Geometry</span><br><span class="paper-authors">Anna Maksymchuk e Iaroslav Sagan</span></td></tr>
  </tbody>
</table>

### <span id="sci">SCI — Sexta-feira, 14:00</span>

<table class="session-table">
  <tbody>
    <tr><td class="session-time">14:00</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Configuration-Aware Runtime Monitoring: Protecting Workflows by Automating the Creation of Monitoring Rules from a Workflow's Configuration File</span><br><span class="paper-authors">Tiago Silva, Sofia Vaz, Vitor Cunha e Daniel Corujo</span></td></tr>
    <tr><td class="session-time">14:20</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">In-network computing anomaly detection for the energy sector</span><br><span class="paper-authors">Martim Carvalho, Tiago Barros, Rui Silva, David Santos, Daniel Corujo, Fatma Marzouk e Rui Aguiar</span></td></tr>
    <tr><td class="session-time">14:40</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Real-Time License Plate Recognition on Embedded Devices for Surveillance Applications</span><br><span class="paper-authors">Sebastião Ferreira e Mário Véstias</span></td></tr>
    <tr><td class="session-time">15:00</td><td class="session-type session-article">Artigo</td><td><span class="paper-title">Lightweight Edge System for Real-Time Object Detection and Tracking</span><br><span class="paper-authors">Paulo Tavares e Mário Véstias</span></td></tr>
  </tbody>
</table>
